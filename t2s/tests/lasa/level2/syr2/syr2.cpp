...
int main()
{
    ...
    // Type of the data to process in C and T2
    #define CTYPE float
    ...
    uX_T(P) = X(total_j);
    uY_T(P) = select(jj == 0, Y(total_i), uY_T(P_jj_minus_1));
    ...
    uX_T.set_bounds(jj, 0, JJ, ii, 0, II)
        .set_bounds(j,  i, J-i, i, 0, I);

    // Create a systolic array
    uX.space_time_transform(jj);
    uX_T.space_time_transform(jj);

    URE Add("Add");
    Add(P) = uZ(P) + uZ_T(P) + A(total_j, total_i);
    Add.set_bounds(jj, 0, JJ,  ii, 0, II)
       .set_bounds(j,  i, J-i, i, 0, I);
